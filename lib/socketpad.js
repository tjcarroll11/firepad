...
firepad.Socketpad = (function(global) {
  ...
  var SocketAdapter = firepad.SocketAdapter;
  ...
  function Socketpad(socket, place, options) {
    if (!(this instanceof Socketpad)) { return new Socketpad(socket, place, options); }
    ...
    if (CodeMirror && place instanceof CodeMirror) {
      ...
      if (curValue !== '') {
        throw new Error("Can't initialize Socketpad with a CodeMirror instance that already contains text.");
      }
    } else if (ace && place && place.session instanceof ace.EditSession) {
      this.ace_ = this.editor_ = place;
      curValue = this.ace_.getValue();
      if (curValue !== '') {
        throw new Error("Can't initialize Socketpad with an ACE instance that already contains text.");
      }
    } else {
    ...
    var userId = this.getOption('userId', Math.round(Math.random() * 100000).toString());
    var userColor = this.getOption('userColor', colorFromUserId(userId));

    this.entityManager_ = new EntityManager();

    this.socketAdapter_ = new SocketAdapter(socket, userId, userColor);
    if (this.codeMirror_) {
    ...
    this.client_ = new EditorClient(this.socketAdapter_, this.editorAdapter_);

    var self = this;
    this.socketAdapter_.on('cursor', function() {
    ...
    this.socketAdapter_.on('ready', function() {
      self.ready_ = true;
  ...
  utils.makeEventEmitter(Socketpad);

  // For readability, these are the primary "constructors", even though right now they're just aliases for Socketpad.
  Socketpad.fromCodeMirror = Socketpad;
  Socketpad.fromACE = Socketpad;

  Socketpad.prototype.dispose = function() {
  ...
  Socketpad.prototype.setUserId = function(userId) {
    this.socketAdapter.setUserId(userId);
  };

  Socketpad.prototype.setUserColor = function(color) {
    this.socketAdapter.setColor(color);
  };

  Socketpad.prototype.getText = function() {
  ...
  Socketpad.prototype.setText = function(textPieces) {
  ...
  Socketpad.prototype.insertTextAtCursor = function(textPieces) {
    this.insertText(this.codeMirror_.indexFromPos(this.codeMirror_.getCursor()), textPieces);
  };

  Socketpad.prototype.insertText = function(index, textPieces) {
  ...
  Socketpad.prototype.getOperationForSpan = function(start, end) {
  ...
  Socketpad.prototype.getHtml = function() {
    return this.getHtmlFromRange(null, null);
  };

  Socketpad.prototype.getHtmlFromSelection = function() {
  ...
  Socketpad.prototype.getHtmlFromRange = function(start, end) {
  ...
  Socketpad.prototype.orderedList = function() {
    this.richTextCodeMirror_.toggleLineAttribute(ATTR.LIST_TYPE, 'o');
    this.codeMirror_.focus();
  };

  Socketpad.prototype.unorderedList = function() {
    this.richTextCodeMirror_.toggleLineAttribute(ATTR.LIST_TYPE, 'u');
    this.codeMirror_.focus();
  };

  Socketpad.prototype.todo = function() {
    this.richTextCodeMirror_.toggleTodo();
    this.codeMirror_.focus();
  };

  Socketpad.prototype.newline = function() {
    this.richTextCodeMirror_.newline();
  };

  Socketpad.prototype.deleteLeft = function() {
    this.richTextCodeMirror_.deleteLeft();
  };

  Socketpad.prototype.deleteRight = function() {
    this.richTextCodeMirror_.deleteRight();
  };

  Socketpad.prototype.indent = function() {
    this.richTextCodeMirror_.indent();
    this.codeMirror_.focus();
  };

  Socketpad.prototype.unindent = function() {
    this.richTextCodeMirror_.unindent();
    this.codeMirror_.focus();
  };

  Socketpad.prototype.undo = function() {
    this.codeMirror_.undo();
  };

  Socketpad.prototype.redo = function() {
    this.codeMirror_.redo();
  };

  Socketpad.prototype.insertEntity = function(type, info, origin) {
    this.richTextCodeMirror_.insertEntityAtCursor(type, info, origin);
  };

  Socketpad.prototype.insertEntityAt = function(index, type, info, origin) {
    this.richTextCodeMirror_.insertEntityAt(index, type, info, origin);
  };

  Socketpad.prototype.registerEntity = function(type, options) {
    this.entityManager_.register(type, options);
  };

  Socketpad.prototype.getOption = function(option, def) {
    return (option in this.options_) ? this.options_[option] : def;
  };

  Socketpad.prototype.assertReady_ = function(funcName) {
    if (!this.ready_) {
      throw new Error('You must wait for the "ready" event before calling ' + funcName + '.');
    }
    if (this.zombie_) {
      throw new Error('You can\'t use a Socketpad after calling dispose()!  [called ' + funcName + ']');
    }
  };

  Socketpad.prototype.makeImageDialog_ = function() {
    this.makeDialog_('img', 'Insert image url');
  };

  Socketpad.prototype.makeDialog_ = function(id, placeholder) {
   var self = this;
  ...
  Socketpad.prototype.addToolbar_ = function() {
  ...
  Socketpad.prototype.addPoweredByLogo_ = function() {
  ...
  Socketpad.prototype.initializeKeyMap_ = function() {
    function binder(fn) {
      return function(cm) {
...
// Export Text classes
firepad.Socketpad.Formatting = firepad.Formatting;
firepad.Socketpad.Text = firepad.Text;
firepad.Socketpad.Entity = firepad.Entity;
firepad.Socketpad.LineFormatting = firepad.LineFormatting;
firepad.Socketpad.Line = firepad.Line;
firepad.Socketpad.TextOperation = firepad.TextOperation;
firepad.Socketpad.Headless = firepad.Headless;

// Export adapters
firepad.Socketpad.RichTextCodeMirrorAdapter = firepad.RichTextCodeMirrorAdapter;
firepad.Socketpad.ACEAdapter = firepad.ACEAdapter;
