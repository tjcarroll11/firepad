...
firepad.SocketAdapter = (function (global) {
  ...
  function SocketAdapter (socket, userId, userColor) {
    this.socket_ = socket;
    this.ready_ = false;
  ...
  SocketAdapter.prototype.sendOperation = function (operation, callback) {
    ...
    this.socket_.emit('operation', {id: revisionId, a: self.userId_, o: operation.toJSON()}, callback);

  };

  SocketAdapter.prototype.sendCursor = function (obj) {
    this.socket_.emit('cursor', obj);
    this.cursor_ = obj;
  };
  ...
  SocketAdapter.prototype.monitorCursors_ = function() {
    function cursorChanged(cursor) {
      self.trigger('cursor', cursor.user, {position: cursor.position, selectionEnd: cursor.selectionEnd}, cursor.color);
    }

    this.socketOn_(this.socket_, 'cursor', cursorChanged);
  };
  ...
  SocketAdapter.prototype.monitorHistory_ = function() {
    ...
    this.socketOn_(this.socket_, 'operation', (operation) => {
      self.pendingReceivedRevisions_[operation.id] = operation;
      if (self.ready_) {
    ...
    fetch(window.location + '&format=ops').then(resp => {
      return resp.JSON();
      // Return array of ops where the first one is a checkpoint.
    }).then(ops => {
      ops.forEach((operation) => {
        // TODO use separate variable to avoid long load during heavy concurrent edit
        self.pendingReceivedRevisions_[operation.id] = { o: operation.o, a: operation.a };
      })
      self.handleInitialRevisions_();
    })
  };

  SocketAdapter.prototype.handleInitialRevisions_ = function() {
    assert(!this.ready_, "Should not be called multiple times.");
    var self = this;
    if(self.pendingReceivedRevisions_.length > 0) {
      // Compose the checkpoint and all subsequent revisions into a single operation to apply at once.
      this.revision_ = revisionFromId(this.pendingReceivedRevisions_[0].id);
